...
public class OrderDecoder {
    ...
    public OrderDecoder(KafkaPublisher kafkaBus) {
    ...
    public List<String> parse(int receivedBytes, byte[] buffer) {
        ...
        for (int delimiter_index = 0; delimiter_index <= receivedBytes - 1; delimiter_index++) {
            if (buffer[delimiter_index] == this.MESSAGE_DELIMITER_BYTE
                ...
                String message = new String(buffer, messageStart, delimiter_index - messageStart, StandardCharsets.US_ASCII);     // parse message from last delimiter to current delimiter
                result.add(message);
